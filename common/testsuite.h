...
static void mavlink_test_actuator_outputs_drl(uint8_t system_id, uint8_t component_id, mavlink_message_t *last_msg)
{
#ifdef MAVLINK_STATUS_FLAG_OUT_MAVLINK1
    mavlink_status_t *status = mavlink_get_channel_status(MAVLINK_COMM_0);
        if ((status->flags & MAVLINK_STATUS_FLAG_OUT_MAVLINK1) && MAVLINK_MSG_ID_ACTUATOR_OUTPUTS_DRL >= 256) {
            return;
        }
#endif
    mavlink_message_t msg;
        uint8_t buffer[MAVLINK_MAX_PACKET_LEN];
        uint16_t i;
    mavlink_actuator_outputs_drl_t packet_in = {
        { 17.0, 18.0, 19.0, 20.0 },53
    };
    mavlink_actuator_outputs_drl_t packet1, packet2;
        memset(&packet1, 0, sizeof(packet1));
        packet1.usedrl = packet_in.usedrl;
        
        mav_array_memcpy(packet1.output, packet_in.output, sizeof(float)*4);
        
#ifdef MAVLINK_STATUS_FLAG_OUT_MAVLINK1
        if (status->flags & MAVLINK_STATUS_FLAG_OUT_MAVLINK1) {
           // cope with extensions
           memset(MAVLINK_MSG_ID_ACTUATOR_OUTPUTS_DRL_MIN_LEN + (char *)&packet1, 0, sizeof(packet1)-MAVLINK_MSG_ID_ACTUATOR_OUTPUTS_DRL_MIN_LEN);
        }
#endif
        memset(&packet2, 0, sizeof(packet2));
    mavlink_msg_actuator_outputs_drl_encode(system_id, component_id, &msg, &packet1);
    mavlink_msg_actuator_outputs_drl_decode(&msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);

        memset(&packet2, 0, sizeof(packet2));
    mavlink_msg_actuator_outputs_drl_pack(system_id, component_id, &msg , packet1.usedrl , packet1.output );
    mavlink_msg_actuator_outputs_drl_decode(&msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);

        memset(&packet2, 0, sizeof(packet2));
    mavlink_msg_actuator_outputs_drl_pack_chan(system_id, component_id, MAVLINK_COMM_0, &msg , packet1.usedrl , packet1.output );
    mavlink_msg_actuator_outputs_drl_decode(&msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);
    ...
    mavlink_msg_actuator_outputs_drl_decode(last_msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);
        
        memset(&packet2, 0, sizeof(packet2));
    mavlink_msg_actuator_outputs_drl_send(MAVLINK_COMM_1 , packet1.usedrl , packet1.output );
    mavlink_msg_actuator_outputs_drl_decode(last_msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);
}
...
static void mavlink_test_common(uint8_t system_id, uint8_t component_id, mavlink_message_t *last_msg)
{
    ...
    mavlink_test_actuator_outputs_drl(system_id, component_id, last_msg);
